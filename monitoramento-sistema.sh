...
function monitorar_logs() {
	sudo grep -E "(fail(ed)?|error|denied|unauthorized)" /var/log/syslog | awk '{print $1, $2, $3, $4, $5, $6, $7}' > $LOG_DIR/monitoramento_logs_sistema.txt
 	sudo grep -E "(fail(ed)?|error|denied|unauthorized)" /var/log/auth.log | awk '{print $1, $2, $3, $4, $5, $6, $7}' > $LOG_DIR/monitoramento_logs_auth.txt
}
...
function monitorar_disco(){
	...
	du -sh /home/maluzeira >> $LOG_DIR/monitoramento_disco.txt
}
...
function monitorar_hardware(){
	...
	iostat | grep -E "Debice|^sda|^sdb|^sdc" | awk '{print $1, $2, $3, $4}' >> $LOG_DIR/monitoramento_hardware.txt
